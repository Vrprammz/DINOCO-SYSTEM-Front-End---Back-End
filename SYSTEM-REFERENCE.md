...
| Snippet 16: Backorder System | V.1.1 | pending | **NEW — Opaque Accept + Admin Split BO** (FEATURE-SPEC-B2B-BACKORDER-2026-04-16). Custom tables: `wp_dinoco_order_attempt_log` (audit, 90d retention) + `wp_dinoco_bo_queue` (BO tracking). 8 REST endpoints `/b2b/v1/bo-*` (split/confirm-full/reject/undo/fulfill/cancel-item/queue/restock-scan). 4 Flex builders (admin bucket-indicator review, customer partial combined, BO ready, BO cancelled). 6 cron jobs (restock_scan 15min + eta_warn daily + pending_review_expire hourly + enumeration_scan + attempt_log_cleanup chunked + daily counter reset). Hooks: `rest_pre_dispatch` priority 5 gate place-order (hard caps qty≤500/item + items≤50 + rate 10/hr + 50/day + daily qty 2000/SKU + tier value cap + unique-SKU/day 20 + suspicious qty flagger). Security: admin Flex bucket (no exact qty leak), CSRF nonce/JWT, artificial jitter 50-150ms, undo 1 max per order, REST meta filter (`_b2b_stock_snapshot` + `_b2b_enumeration_flags` hidden from non-admin). Master flag `b2b_flag_bo_system` default OFF. Spec verdict NEEDS_REVISION (4 CRIT + 5 HIGH security + 2 CRIT + 5 HIGH DB) — all CRIT + most HIGH applied in V.1.1. |
