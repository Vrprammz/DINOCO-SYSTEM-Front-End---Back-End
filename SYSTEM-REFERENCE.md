...
| [Admin System] DINOCO SN Go-Live Tool | V.1.0 | 1215 | `[dinoco_sn_golive]` | **NEW (2026-05-15)** — No-SSH activation wizard. 5-stage UI (Preflight 9-gate / Schema Install / Role Matrix 50-user × 4-role / Flag Control 12-flag whitelist / Monitor 24h). REST `/dinoco-sn-golive/v1/*` (7 endpoints). Security: `manage_options` + `wp_rest` nonce + hardcoded flag whitelist (gates arbitrary option writes) + rate limit + Flag Audit Log integration + `autoload=false` on update_option (Round 9). Designed TEMPORARY — delete after Go-Live stable (mirror Flash V.42 Go-Live pattern). Mirrors `[Admin System] Flash Shipping V.42 Go-Live Tool` V.2.1 structure. |
...
| [System] DINOCO SN REST API | V.0.51 | 1196 | -- | **NEW (v2.13 Phase 1, 2026-05-04)** — Production S/N Management REST namespace `/dinoco-sn/v1/*` (34+ endpoints). Phase 0-3 coverage: batches CRUD + receive (bulk D4 contract) + lookup (PII-stripped, 60s cache) + sc-lookup (warehouse cap) + activate + audit + void/swap/recall/reissue (4-eyes tier matrix) + LTV + geo heatmap + stolen registry + demand forecast + marketplace (extension checkout / pricing / pending-review / transactions / receipt render). Bulk admin wizard (plan §K.4) 5/5 endpoints. Idempotency-Key wrapper includes `actor_user_id` (Round 30+ pattern). Phase 3 audit + R3-R12 rounds closed 50+ findings. **V.0.47 (2026-05-14)**: marketplace `/checkout` wires referral redemption. **V.0.48 (2026-05-14)**: OP-3 bulk endpoints. **V.0.49 (2026-05-15)**: Phase 6 W19+ Gap-2 closure — `dinoco_sn_fire_agent_webhook` listener @ priority 8 on `dinoco_sn_pool_status_changed` POSTs to OpenClaw `/webhook/sn-event` with bearer `LIFF_AI_AGENT_KEY` (non-blocking + Throwable catch). Flag-gated `dinoco_sn_agent_webhook_enabled` default OFF. **V.0.50 (2026-05-15)**: NEW `/marketplace/{id}/receipt/render` HTML endpoint replaces W17 PDF stub (plain ใบเสร็จธรรมดา non-VAT, browser print → Save as PDF). **V.0.51 (2026-05-15)**: DINOCO logo branding strip on receipt via 3-tier resolver `dinoco_brand_logo_url('dark')` → `b2b_get_logo_url()` → hardcoded fallback. |
...
| [System] DINOCO Claim Payment LIFF | V.0.12 **DEPRECATED** | 1212 | `[dinoco_claim_pay]` | **DEPRECATED (Sprint 32, 2026-05-14)** — Standalone `/claim-pay/?claim_id=X&charge_id=Y` LIFF page collapsed into Member Dashboard. Now serves redirect-only behavior: incoming Flex deep-links 301 → `/dashboard/?action=pay&claim_id=X&charge_id=Y#claim-card-{id}` (Sprint 33 closed 5 BLOCKERs from dual UX audit). Standalone slip-upload + Slip2Go verify logic still hot for legacy LINE Flex payloads pre-2026-05-14. Removal pending 60-day customer cache TTL. **V.0.12 (R56 2026-05-15)**: NEW direct wrap on `/charges/{id}/approve-refund` (CRITICAL 4-eyes audit op) — closes 🎯 85% idempotency milestone via Round 56 batch 34 (cluster expansion 2→9 endpoints). |
| [System] DINOCO Warranty Extension Marketplace | V.0.6 | 1202 | `[dinoco_warranty_extend]` | **NEW (v2.13 Phase 5, 2026-05-07)** — Customer-facing LIFF `/warranty/extend?sn=DNCSS...` for warranty extension purchase. 4-stage UI flow (plan select → payment instructions → slip upload → success/pending). LINE OAuth reuse pattern (no new JWT) mirror Activation LIFF. **V.0.6 (boss 2026-05-15)**: NON-VAT บัญชีบุคคล scope refactor — `dinoco_sn_mpx_compute_total()` VAT calc REMOVED ($vat_rate param retained for backward-compat but value forced to 0). LIFF UI "VAT 7%" row removed. F#8 effort 80h → 30h (drops legal workstream / tax invoice format). Plain receipt only (ใบเสร็จธรรมดา) — NO ใบกำกับภาษี. Memory: `feedback_non_vat_personal_bank.md`. |
...
| [Admin System] DINOCO Production SN Manager | V.0.61 | 1195 | `[dinoco_admin_production_sn]` | **NEW (v2.13 Phase 0-3, 2026-05-04..05)** — Foundation snippet for Production S/N Management System. Schema lazy-install on `admin_init` (15 tables incl. sn_pool split hot/cold + audit + 11 supporting). 9 admin tabs (Batches/รับเพลท/Pool/จัดการ/Audit/VIP/Fraud-CUT/Geo/Stolen + Marketplace) embedded in Inventory DB tab 8 via `do_shortcode`. Module Registry self-registration (section=inventory, order=25). 5 feature flags default OFF (`dinoco_sn_system_enabled` master). 3 capabilities (`dinoco_sn_warehouse` / `dinoco_sn_approver` / `dinoco_sn_view_pii`). Hierarchy resolver `dinoco_sn_required_plates_for_sku()` + DD-3 array_unique. **V.0.17**: Flex template builders. **V.0.18 (Phase 2 W7)**: 6 Member Dashboard read-only helpers. 14 cron jobs heartbeat-tracked. **R3-R12 audit hardening**: HMAC URL signing + canonical idempotency hash + unified lock key `dnc_sn_{md5(sn)}` + cron heartbeat in `finally` + obs_capture signature fix (54 sites) + per-user LTV transient invalidation. **V.0.59 (2026-05-13)**: QW-7 Smart Service Reminder Flex builder + cron 02:25 ICT (educational, NO promo_code). **V.0.60 (2026-05-14)**: QW-5 finish — promo_codes schema adds `referrer_user_id` column + `code` VARCHAR(40) widening for REF audit format. **V.0.61 (2026-05-15)**: Phase 6 W19+ Tab 8 F#13 Geographic Heatmap — Leaflet 1.9.4 lazy-load from unpkg CDN replaces "Phase 4 W12 wires" placeholder. Circle markers (radius ∝ √activations, red for gray-market) + OSM tiles + Thailand-centered fitBounds + marker popup → drill-down sidebar. |
