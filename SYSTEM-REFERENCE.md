...
| Snippet 16: Backorder System | V.1.2 | pending | **NEW — Opaque Accept + Admin Split BO** (FEATURE-SPEC-B2B-BACKORDER-2026-04-16). Custom tables: `wp_dinoco_order_attempt_log` (audit, 90d retention) + `wp_dinoco_bo_queue` (BO tracking). 8 REST endpoints `/b2b/v1/bo-*` (split/confirm-full/reject/undo/fulfill/cancel-item/queue/restock-scan). 4 Flex builders. 6 cron jobs. Place-order gate (hard caps + rate limits + unique-SKU/day + suspicious qty flagger + jitter 50-150ms). Security: admin Flex bucket (no exact qty leak), CSRF nonce/JWT, undo 1 max per order, REST meta filter. **V.1.2 Admin UI**: shortcode `[b2b_bo_admin]` (Pending Review + Backorders tabs + Split Review modal + action buttons) + `[b2b_bo_flags]` (toggle 3 flags UI + config viewer). Master flag `b2b_flag_bo_system` default OFF. Spec verdict NEEDS_REVISION (4 CRIT + 5 HIGH security + 2 CRIT + 5 HIGH DB) — all CRIT + most HIGH applied in V.1.1. |
