...
|------|---------|-------|-------------|
| Snippet 1: Core Utilities & LINE Flex Builders | V.33.7 | 72 | LINE push, Flex templates, HMAC URL, bank helpers + **V.33.7 security C2 fix**: `b2b_rate_limit()` atomic via MySQL GET_LOCK. **V.33.6 polish**: M3 refactor ใช้ `b2b_get_product_data()`. **V.33.5 Phase 0 Hotfix**: `b2b_check_order_oos()` hierarchy-aware (Ticket #6266) |
...
| Snippet 16: Backorder System | V.1.6 | pending | **Opaque Accept + Admin Split BO** (FEATURE-SPEC-B2B-BACKORDER-2026-04-16). 14 REST endpoints + 4 Flex + 6 cron + 4 shortcodes (`[b2b_bo_admin]`, `[b2b_bo_flags]`, `[b2b_bo_security_log]`, `[b2b_bo_customer_order_detail]`). **V.1.6 gap fixes** (architect review 4 CRIT + 6 HIGH + 2 MEDIUM): C1 place-order hook wired (Snippet 3 V.41.4) + C2 confirm_order BO gate (Snippet 2 V.34.4) + C3 Split BO deep-link + H1 badge updater (Dashboard V.32.5) + H2 /bo-order-detail + H3 /bo-update-eta + H4 bulk ops + H5 Flash hook `b2b_bo_items_fulfilled` + H6 print queue hook + M3 debt per-SKU compound + M6/M7 customer Flex footer buttons. Master flag `b2b_flag_bo_system` default OFF. ~3497 LOC. |
...
| POST | `/phase4-migration` | Admin + LIFF | Delegate to `b2f_phase4_run_classification_migration($dry_run, $batch)`. Guards: `b2f_phase4_migration_in_progress` lock (503), schema V10 activated (409). Dry-run returns `csv_url` (wp-content/b2f-backups/). Body: `{confirm, dry_run, batch_size?}`. Rate **5/HOUR** (heavy op). |
...
**Rate limit**: 20 req/hour/user per read endpoint (ผ่าน `b2b_rate_limit()`); 5/hour for destructive actions (activate-schema, backfill, phase4-migration); 5/min for audit mutations.
