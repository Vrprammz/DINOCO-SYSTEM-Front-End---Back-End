...
| `manual_hold_by` | INT UNSIGNED | NULL | Admin user ID ที่กด hold (FK → wp_users.ID) |
| `stock_updated_at` | DATETIME | NULL | stock_qty แก้ล่าสุดเมื่อไหร่ (ไม่ต้อง query transaction log) |
...
CREATE TABLE IF NOT EXISTS wp_dinoco_stock_transactions (
    ...
    type ENUM('b2f_receive','b2b_reserved','b2b_shipped','b2b_cancel_return','manual_add','manual_subtract','dip_stock_adjust','initial_set') NOT NULL,
    ...
    batch_id VARCHAR(50) DEFAULT NULL COMMENT 'Group transactions (Dip Stock approve / B2F receive PO)',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    KEY idx_sku_created (sku, created_at DESC) COMMENT 'Stock Detail page sorted query',
...
CREATE TABLE IF NOT EXISTS wp_dinoco_dip_stock (
    ...
    snapshot_at DATETIME DEFAULT NULL COMMENT 'เวลาที่ snapshot expected_qty (ตรวจ stock movement หลัง snapshot)',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
└── เพิ่ม transition: awaiting_confirm → cancelled (actor: system) สำหรับ auto-cancel 30 นาที

[B2B] Snippet 2: LINE Webhook Gateway & Order Creator (DB_ID: 51)
├── Hook b2b_order_status_changed (priority 5): stock deduction เมื่อ awaiting_confirm
├── Auto-cancel handler: guard is_walkin + ใช้ _awaiting_confirm_at meta (ไม่ใช่ post_date)
├── Cancel handler: คืนสต็อก (ทุกกรณีที่มี _stock_deducted)
...
5. FSM: เพิ่ม awaiting_confirm → cancelled ก่อน deploy auto-cancel
```

### ⚠️ Production Deploy Order (ลำดับสำคัญมาก!)

```
Phase 1 Deploy ต้องทำตามลำดับนี้:

Step 1: Database (ไม่กระทบ production)
├── dbDelta → เพิ่ม columns ใหม่ (stock_qty=0 ทุกตัว)
├── CREATE TABLE stock_transactions, dip_stock, dip_stock_items
└── set wp_option dinoco_inv_initialized = false

Step 2: FSM Update (ไม่กระทบ production)
└── Snippet 14: เพิ่ม awaiting_confirm → cancelled transition

Step 3: OOS Migration (ทำก่อนปิด cron!)
├── Query SKU ที่ stock_status=out_of_stock + oos_duration_hours>0
├── Set manual_hold=1 + reason="migrated from OOS timer"
├── Copy oos_eta_date → bo_eta_override
├── ✅ ตรวจว่า migration ถูก
├── แล้วค่อยปิด b2b_oos_expiry_check cron
└── แล้วค่อยลบ inline auto-expire (Snippet 3:422-433)

Step 4: Deploy Stock Functions + Hooks (เริ่มทำงาน)
├── dinoco_stock_add/subtract/auto_status
├── Hook priority 5 บน b2b_order_status_changed
├── Auto-cancel handler (guard is_walkin + _awaiting_confirm_at meta)
├── B2F receive-goods → dinoco_stock_add()
├── Dual-write: auto_status() sync postmeta + delete_transient
└── ⚠️ auto_status ยัง SKIP เพราะ initialized=false

Step 5: Initial Stock Count (Admin ทำ)
├── Admin ทำ Dip Stock ครั้งแรก (กรอกจำนวนจริง)
├── Approve → stock_qty ถูกต้อง
└── Admin กดปุ่ม "เปิดระบบสต็อก" → dinoco_inv_initialized=true

Step 6: Auto-status เริ่มทำงาน
└── ตั้งแต่นี้ stock_status auto-update ตาม stock_qty
```
...
**⚠️ FSM PREREQUISITE**: ต้องเพิ่ม transition ใน B2B FSM (Snippet 14) ก่อน:
```php
// เพิ่มใน $transitions['awaiting_confirm']:
'awaiting_confirm' => array(
    'awaiting_payment'  => 'customer',
    'cancelled'         => 'system',    // ← เพิ่มใหม่: auto-cancel 30 min timeout
    'cancel_requested'  => 'customer',
    'change_requested'  => 'customer',
),
```
ถ้าไม่เพิ่ม → `b2b_set_order_status($id, 'cancelled')` จะถูก FSM block
...
เมื่อ new_status = 'awaiting_confirm' (priority 5 — ก่อน walkin_auto_complete priority 10):
├── ตัดสต็อก: parse order text ด้วย regex /\(([A-Za-z0-9_\-]+)\)\s*x(\d+)/i
...
Auto-cancel fallback (DISABLE_WP_CRON):
├── DINOCO ใช้ DISABLE_WP_CRON=true + external cron
├── wp_schedule_single_event เก็บใน wp_options(cron) ปกติ
├── ต้องเพิ่ม meta `_awaiting_confirm_at` = current_time('mysql') ตอน transition
│   (ห้ามใช้ post_date เพราะมันคือเวลาสร้าง order ไม่ใช่เวลาเข้า awaiting_confirm)
├── ถ้า external cron trigger ไม่ทัน → เพิ่ม lazy check ใน **admin endpoint เท่านั้น**:
│   (ไม่ใส่ใน catalog endpoint เพราะจะเพิ่ม overhead ตัวแทน)
│   ถ้า status = awaiting_confirm + _awaiting_confirm_at > 30 นาที → auto cancel
└── Lazy check = safety net ไม่ต้องพึ่ง cron timing 100%
...
└── Guard: auto-cancel handler ตรวจ is_walkin → skip
...
```sql
ALTER TABLE wp_dinoco_products
  ADD INDEX idx_active_stock (is_active, stock_status, stock_qty);
-- หมายเหตุ: idx_sku_active ไม่ต้องเพิ่ม — sku มี UNIQUE KEY อยู่แล้ว
```
...
function dinoco_stock_auto_status( $sku ) {
    // Guard: ถ้ายังไม่เคย initialize (Dip Stock ครั้งแรก) → skip
    if ( ! get_option( 'dinoco_inv_initialized', false ) ) return;
    
    global $wpdb;
    $table = $wpdb->prefix . 'dinoco_products';
    // รวม query เดียว (ลด race condition + performance)
    ...
    // ถ้า manual_hold = 1 → force out_of_stock (Admin ล็อกเอง)
    if ( (int) $product->manual_hold ) $new_status = 'out_of_stock';
...
> `order_items` เป็น plain text → parse ด้วย regex `/\(([A-Za-z0-9_\-]+)\)\s*x(\d+)/i`
...
function dinoco_stock_deduct_for_order( $order_id ) {
    ...
    foreach ( $matches as $m ) {
        $sku = strtoupper( trim( $m[1] ) ); // normalize SKU (A-5)
        $qty = intval( $m[2] );
        if ( $qty <= 0 ) continue;
        
        if ( isset( $relations[ $sku ] ) && ! empty( $relations[ $sku ] ) ) {
            // Set product: validate ทุก child อยู่ใน catalog ก่อน (E-4)
            $all_valid = true;
            foreach ( $relations[ $sku ] as $child_sku ) {
                if ( ! DINOCO_Catalog::get_by_sku( $child_sku ) ) {
                    $all_valid = false;
                    if ( function_exists( 'b2b_log' ) ) b2b_log( "[Stock] WARNING: child SKU {$child_sku} not in catalog, skip set {$sku}" );
                }
            }
            if ( ! $all_valid ) continue; // skip ทั้ง set ถ้า child ไม่ครบ
    ...
    update_post_meta( $order_id, '_awaiting_confirm_at', current_time( 'mysql' ) ); // E-6: เวลาจริง
    delete_transient( 'b2b_sku_data_map' ); // DD-5
    
    // Schedule auto-cancel 30 นาที (ไม่ schedule ถ้า walk-in)
...
### Flex Message: แก้ Flex "สินค้าพร้อมจัดส่ง" เดิม — function: `b2b_build_flex_stock_confirmed` (Snippet 1:1387)

```
Flex ที่ต้องแก้: "✅ สินค้าพร้อมจัดส่ง" (b2b_build_flex_confirm_bill)
ใน Snippet 1 — Flex ที่ส่งไปกลุ่มตัวแทนตอน awaiting_confirm

แก้เพิ่มข้อความใต้ "กรุณากดยืนยันบิลเพื่อดำเนินการต่อครับ":

+  "⏰ สินค้าล็อกไว้ให้ 30 นาที"
+  "กรุณายืนยันก่อนหมดเวลา"
+  "หากไม่ยืนยัน รายการจะถูกยกเลิกอัตโนมัติ"

สีข้อความ: สีส้ม/เหลืองเตือน (#f59e0b) เพื่อให้เห็นชัด
ขนาด: xs-sm (ไม่ใหญ่เกิน แต่เห็นได้)

ไม่แก้ปุ่ม ไม่แก้ layout — แค่เพิ่มข้อความเตือนเวลา
```

```
Flex auto-cancel (สร้างใหม่ — ส่งเมื่อหมด 30 นาที):

┌──────────────────────────────────┐
│ ⏰ หมดเวลายืนยัน                 │  (header สีแดง)
│ Ticket #5955                     │
├──────────────────────────────────┤
│ Crash Bar IRON ... (DNCADV009)   │
│ x1 = ฿2,240                     │
│                                  │
