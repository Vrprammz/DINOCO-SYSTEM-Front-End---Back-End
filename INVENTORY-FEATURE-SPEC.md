...
Version: 2.6 | Date: 2026-04-04 | Author: Feature Architect + Fullstack + Tech Lead + DB Expert Review
...
├── ถ้า external cron trigger ไม่ทัน 30 นาที → เพิ่ม lazy check:
│   ทุกครั้งที่ query order (admin view / catalog load):
│   ถ้า status = awaiting_confirm + post_date > 30 นาที → auto cancel + คืนสต็อก
...
function dinoco_stock_auto_status( $sku ) {
    ...
    $hold = (int) $wpdb->get_var( $wpdb->prepare(
        "SELECT manual_hold FROM {$table} WHERE sku = %s", $sku
    ) );
    if ( $hold ) $new_status = 'out_of_stock';
