...
| Metric | Count |
|--------|-------|
| Total integrated endpoints | **109** (+5 new — Round 44 batch 22: flash-test/run-step + flash-test/simulate-webhook + api-keys/generate + api-keys/revoke + discount-mapping. Push toward 60% milestone — 109/196 = 55.6%. NEW Brand Voice namespace entry — 2 endpoints, first idempotency wraps for Brand Voice cluster) |
...
| Coverage | **109 / 196 = 55.6%** of POST endpoints — advancing toward 60% milestone. **26-round sustained Idempotency-Key campaign Rounds 18-44**. Round 44 closes Flash test sysadmin tools (run-step + simulate-webhook) + Brand Voice security ops (api-keys/generate + api-keys/revoke) + B2B legacy backwards-compat write path (discount-mapping bulk upsert). 3 cross-snippet files modified: `[B2B] Snippet 9` V.34.3→V.34.4 + `[Admin System] DINOCO Brand Voice Pool` V.2.11→V.2.12 + `[B2B] Snippet 6` V.31.3→V.31.4. **B2B namespace coverage = 53 endpoints** (+3 since Round 43 = 50 → 53 — flash-test/run-step + flash-test/simulate-webhook + discount-mapping). **Inventory namespace coverage = 17 endpoints** (unchanged). **MCP cluster coverage = 13/17 = ~76%** (saturated since Round 35). **B2F namespace coverage = 21 endpoints** (unchanged). **LIFF AI namespace coverage = 3 endpoints** (unchanged). **Brand Voice namespace coverage = 2 endpoints** (NEW R44 — first Brand Voice idempotency wraps; api-keys/generate + api-keys/revoke). |
| Cumulative test cases | 415 (Round 19-44 — Round 44 added 16) |
| Body-shape distinct hashes asserted | 108 (Round 44: +5 new — all unique; flash-test/run-step single {ticket_id, step} step enum discriminator; flash-test/simulate-webhook single {ticket_id, type} type enum discriminator; api-keys/generate single {name} security ops; api-keys/revoke single {index} security event guard; discount-mapping bulk-shape items[] sorted by SKU UPPER + per-row normalized — admin business decision change mid-retry → 409) |
...
## Integrated endpoints (109) — Round 44 push toward 60% milestone
...
## Pending POST endpoints (Round 45+ candidates)
