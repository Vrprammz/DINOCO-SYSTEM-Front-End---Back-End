...
| Metric | Count |
|--------|-------|
| Total integrated endpoints | **104** (+5 new — Round 43 batch 21: invoice/init + invoice/issue + invoice/record-payment + invoice/record-refund + invoice/cancel — all 5 Manual Invoice retry-prone POST endpoints. Push past 50% MAJOR MILESTONE toward 60% target) |
...
| Coverage | **104 / 196 = 53.1%** of POST endpoints — past 1/2 of POST surface integrated, advancing toward 60% milestone. **25-round sustained Idempotency-Key campaign Rounds 18-43**. Round 43 closes Manual Invoice cluster — 5 long-pending high-value POST endpoints (debt double-clear / debt double-credit guards + invoice number sequence guard + LINE push spam guards). All 5 endpoints same file `[Admin System] DINOCO Manual Invoice System` V.34.10 → V.34.11 — no cross-snippet coordination. **B2B namespace coverage = 50 endpoints** (+5 since Round 42 = 45 → 50). **Inventory namespace coverage = 17 endpoints** (unchanged). **MCP cluster coverage = 13/17 = ~76%** (saturated since Round 35). **B2F namespace coverage = 21 endpoints** (unchanged). **LIFF AI namespace coverage = 3 endpoints** (unchanged). |
| Cumulative test cases | 399 (Round 19-43 — Round 43 added 17) |
| Body-shape distinct hashes asserted | 103 (Round 43: +5 new — all unique; constant-marker pattern 5th instance via invoice/init {action: 'init', user_id} alongside R30 stock/initialize + R32 manual-flash-test + R39 daily-summary + R40 dip-stock/start; record-payment + record-refund single-shape with float-precision normalize via amount round(2); cancel boolean discriminator force=0 vs force=1 different audit path) |
...
## Integrated endpoints (104) — Round 43 push past 50% milestone toward 60%
...
## Pending POST endpoints (Round 44+ candidates)
...
>
> **Round 44 recommendation**: Continue toward 60% milestone. Remaining candidate batches
> exist in: B2B Flash test cluster (`flash-test/orders` + `flash-test/run-step` +
> `flash-test/simulate-webhook` + `flash-tracking/{ticket_id}`), B2B admin Brand Voice
> (`api-keys/generate` + `api-keys/revoke`), B2B discount-mapping (legacy backwards-compat
> POST), Inventory `product/shipping/bulk` CSV import + `shipping-compute` dry-run.
> ~3 batches × 5 endpoints = +15 → ~119/196 = ~60.7% milestone Round 46.
...
### High priority (Round 44+ candidates)

> **Round 44+ targets**: B2B Flash test cluster (`flash-test/orders` + `flash-test/run-step` +
> `flash-test/simulate-webhook` — sysadmin tools, low retry rate but quota burn), B2B
> Brand Voice API key management (`api-keys/generate` + `api-keys/revoke` — single-action
> security ops, retry would create extra keys / revoke wrong key), B2B legacy
> `discount-mapping` POST (backwards-compat write path), Inventory `product/shipping/bulk`
> CSV import + `shipping-compute` dry-run resolver (Flash quota dedup useful even for
> dry-run). Aim ~3 batches → ~60% milestone by Round 46.

| Endpoint | Snippet | Risk if double-fired | Round candidate |
|----------|---------|----------------------|-----------------|
| `POST /b2b/v1/flash-test/orders` | `[B2B] Snippet 9` | Flash test order creation double-click → 2× test order + Flash quota burn | Round 44 |
| `POST /b2b/v1/flash-test/run-step` | `[B2B] Snippet 9` | Flash test step runner — sysadmin tool low retry rate | Round 44 |
| `POST /b2b/v1/flash-test/simulate-webhook` | `[B2B] Snippet 9` | Flash webhook simulator — re-run on retry produces 2× state mutations | Round 44 |
| `POST /brand-voice/v1/api-keys/generate` | Brand Voice | Generate API key double-click → 2 keys created (security risk: extra key in wild) | Round 44 |
| `POST /brand-voice/v1/api-keys/revoke` | Brand Voice | Revoke key double-click → revoke wrong key on race (audit log scrambled) | Round 44 |
| `POST /b2b/v1/discount-mapping` | `[B2B] Snippet 6` | Legacy backwards-compat POST — admin double-click batch upsert → 2× custom table writes | Round 44 |
| `POST /dinoco-stock/v1/product/shipping/bulk` | Inventory | CSV bulk import double-click → 2× INSERT/UPDATE batch | Round 45 |
| `POST /dinoco-stock/v1/shipping-compute` | Inventory | Dry-run resolver — Flash quota dedup useful | Round 45 |
...
Total: **399 contract tests** across 21 rounds (Rounds 19-43). Round 29 introduced
`IdempotencyTestFixture` base class — Round 30+ fully adopt it
(`IdempotencyRound43Test.php` averages ~4 LOC/test).
