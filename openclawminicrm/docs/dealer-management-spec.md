...
| stock_back | ทุก 1 ชม. | lead ที่ `waiting_stock` + สต็อก SKU กลับมา | แจ้งตัวแทน |
...
T+24h: ถามลูกค้า "ตัวแทนติดต่อหรือยังคะ"
  ...
  ↓ แจ้งตัวแทน (LINE text): "ลูกค้าเปรมแจ้งว่ายังไม่ได้รับการติดต่อ รบกวนติดต่อด้วยนะคะ"
...
ตัวแทนรายงาน: "ลูกค้าอยากได้ XXX แต่ของหมด"
  ...
  ↓ Mayom cron ทุก 1 ชม. เช็คสต็อก (เรียก dinoco_check_stock หรือ Global Inventory DB)
  ↓ พอสต็อก SKU กลับมา (stock > 0):
    → แจ้งตัวแทน (LINE text ไม่ต้อง Flex):
      "สินค้า Set Side Case 37L Silver (DNCGND37LSPROS) กลับมาแล้วค่ะ
       ลูกค้า เปรม ที่เคยสนใจ ลองติดต่อไปดูนะคะ"
...
{
  _id: ObjectId,
  sourceId: String,           // fb_xxx / Cxxx
  platform: String,           // facebook / line / instagram
  customerName: String,
  phone: String,
  productInterest: String,    // "Set Side Case 37L Silver NX500"
  province: String,           // "กรุงเทพ"
  dealerId: ObjectId,         // ref → dealers collection
  dealerName: String,
  status: String,
  waitingSKU: String | null,  // SKU ที่รอสต็อก (UC3)
  notes: [                    // ประวัติ/เหตุผล
    { text: String, by: String, at: Date }
  ],
  closedReason: String | null, // เหตุผลปิด (closed_lost)
  history: [
    { status: String, at: Date, by: String }
  ],
  createdAt: Date,
  updatedAt: Date,
}
