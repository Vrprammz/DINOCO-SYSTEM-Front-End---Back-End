...
- **B2F Snippets** (DB_ID 1160-1171):
  ...
  - Snippet 8 (1168): Admin LIFF E-Catalog V.5.0 — หน้าสั่งซื้อจาก LINE (LIFF auth V.2.0 ไม่ต้อง WP login) + Multi-Currency UI (V.3.0) + V.4.4: JS-side type derivation from `sku_relations` + **V.5.0: DD-3 shared badge ใน SET Detail "🔗 ใช้ใน N ชุด" (`countTopSetsForProduct` walk-up) + `renderCartManufacturingSummary()` เตือน shared leaf ("L1 ต้องผลิต 4 ชิ้น: SET_A×2 + SET_B×1 + เดี่ยว×1")**
