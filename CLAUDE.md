...
- **B2F Hierarchy Support** (V.8.7/V.6.1/V.3.2 — Phase 1 MVP): ระบบ B2F รองรับ 3-Level SKU Hierarchy (แม่-ลูก-หลาน) แล้ว. `create-po` DD-7 auto-expand SET → leaf items พร้อม track `poi_parent_sku` + `poi_parent_name` snapshot ใน po_items repeater (Snippet 0 V.3.2). API `po-detail` return `parent_sku`/`parent_name` per item (Snippet 2 V.8.7). `b2f_group_items_by_set()` helper ใน Snippet 1 V.6.1 group items by SET parent. Flex cards 4 ตัว updated (items_list, po_created, maker_confirmed, delivered_detail) แสดง SET header สีม่วง 🟣 + children indent. PO เก่า backward compat: empty parent_sku = flat list เดิม. DD-3 shared child: เก็บ parent_sku ตัวแรก (MVP). Phase 2: Maker LIFF + Admin Dashboard + PO Image grouping.
