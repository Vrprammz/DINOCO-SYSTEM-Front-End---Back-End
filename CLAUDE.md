...
- **B2F Snippets** (DB_ID 1160-1171):
  ...
  - Snippet 5 (1166): Admin Dashboard Tabs V.4.8 — 3 shortcodes embedded ใน Admin Dashboard + Bulk Cancel PO (V.2.0) + Multi-Currency UI (V.3.0) + Settings REST endpoint + receive form remaining fix (shipped-received) + V.4.8: JS-side computeProductTypes from `sku_relations` (no PHP auto_type/product_type dependency)
  ...
  - Snippet 8 (1168): Admin LIFF E-Catalog V.4.4 — หน้าสั่งซื้อจาก LINE (LIFF auth V.2.0 ไม่ต้อง WP login) + Multi-Currency UI (V.3.0) + V.4.4: JS-side type derivation from `sku_relations` (same as Product Catalog computeProductTypes)
