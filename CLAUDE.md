...
- **B2F Snippets** (DB_ID 1160-1171):
  ...
  - Snippet 8 (1168): Admin LIFF E-Catalog V.5.1 — หน้าสั่งซื้อจาก LINE (LIFF auth V.2.0 ไม่ต้อง WP login) + Multi-Currency UI (V.3.0) + V.4.4: JS-side type derivation from `sku_relations` + V.5.0: DD-3 shared badge ใน SET Detail "🔗 ใช้ใน N ชุด" (`countTopSetsForProduct` walk-up) + `renderCartManufacturingSummary()` เตือน shared leaf + **V.5.1: parseModels normalize object → string (แก้ "[object Object]") + type tabs mutually exclusive + chip count badges + hide empty chips + relabel "สินค้าเป็นชุด"/"แยกชุด (แยกบน-ล่าง)"/"แยกชิ้น (เป็นข้าง)"**
