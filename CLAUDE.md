...
- **B2F Hierarchy Support** (Phase 1+2 Complete): ระบบ B2F รองรับ 3-Level SKU Hierarchy (แม่-ลูก-หลาน) ครบทุก UI. **Phase 1 (Data+Flex)**: `create-po` DD-7 auto-expand SET → leaf items พร้อม track `poi_parent_sku` + `poi_parent_name` snapshot ใน po_items repeater (Snippet 0 V.3.2). API `po-detail` return `parent_sku`/`parent_name` per item + `maker-products` enrich `is_set`/`product_type`/`children_count` (Snippet 2 V.8.7). `b2f_group_items_by_set()` helper ใน Snippet 1 V.6.1 — Flex cards 4 ตัว updated แสดง SET header สีม่วง 🟣. **Phase 2 (All UI)**: E-Catalog badges ม่วง/น้ำเงิน/เขียว (Snippet 8 V.3.1). Maker LIFF confirm/detail/delivery grouped (Snippet 4 V.4.2). PO Ticket View items grouped (Snippet 9 V.3.4). PO Image A4 SET header rows สีม่วง+subtotal (Snippet 10 V.2.6). PO เก่า backward compat: empty parent_sku = flat list เดิม. DD-3 shared child: เก็บ parent_sku ตัวแรก.
