...
- **B2F Snippets** (DB_ID 1160-1171):
  ...
  - Snippet 8 (1168): Admin LIFF E-Catalog V.5.2 — หน้าสั่งซื้อจาก LINE (LIFF auth V.2.0 ไม่ต้อง WP login) + Multi-Currency UI (V.3.0) + V.4.4: JS-side type derivation from `sku_relations` + V.5.0: DD-3 shared badge ใน SET Detail "🔗 ใช้ใน N ชุด" + `renderCartManufacturingSummary()` + V.5.1: parseModels normalize + type tabs mutually exclusive + chip count badges + hide empty + **V.5.2: Align type chips กับ Inventory Stock Management: "ชุด SET" / "เดี่ยว" / "ลูกชิ้นส่วน" / "ชิ้นส่วนย่อย" + เพิ่ม chip "เดี่ยว" (type=single)**
