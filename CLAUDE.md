...
- **Hierarchy Tag System V.42.9-42.14** (Admin Inventory, 2026-04-10): Product Catalog UI overhaul — 4 badge types + hybrid classification + image skeleton:
  ...
  - **V.42.15 Price Suggestion (Phase 2)**: `renderCatChildren` แสดง suggestion badge ข้าง child/grandchild ใน Set Components list. Child = `parent_price / N children` (badge ม่วง 💡). Grandchild = `child_actual_price / M grandchildren` (fallback `parent/N×M` ถ้าลูกไม่มีราคา — badge เขียว). เปรียบเทียบกับ actual price แสดง diff% (`+N%` แดง, `-N%` เขียว) เมื่อ `|diff| >= 1%`. Live update — `input` handler บน `#cat-price` debounced 250ms → re-render children list ทันที. Display-only (ไม่ auto-apply — admin เห็น hint แล้วไปกรอกใน child modal เอง)
