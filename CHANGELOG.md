...
- **NEW** `liff-src/b2f/maker/event-delegation.js` (~165 LOC) — pure DI module. `setupEventDelegation(root, deps)` wires one click listener that dispatches via `data-action` attribute on the closest ancestor of click target. 7 supported actions (navigate / navigate-with-po / deliver-open / deliver-back / deliver-step / deliver-fill-all / deliver-submit). Returns idempotent cleanup fn. Handler exceptions caught + logged via `console.error`. Click bubbling (`closest()`) so inner children of action elements still resolve.
- `liff-src/b2f/maker/entry.js` V.0.4 → V.0.5 — wraps `event-delegation.js` core with imported router + loader handlers. Wires delegation on `#b2f-app` after router setup. **DROPS 7 legacy `window.*` globals**: `goToPage`, `goToPageWithPO`, `b2fOpenDeliverForm`, `b2fFillAllRemaining`, `b2fStepQty`, `b2fSubmitDeliver`, `loadDeliverPage`. **DROPS** `window.DINOCO_B2F_MAKER_RENDERERS`. **KEEPS** single namespaced surface `window.DINOCO_B2F_MAKER` (frozen, debug only — NOT consumed by Snippet 4 inline JS).
- `liff-src/b2f/maker/pages/{detail,reschedule,deliver,list}.js` — all inline `onclick` → `data-action` migration. Visual + behavior identical (REG-029 byte-equivalent).
  - `detail.js` — 3 status-action buttons → `data-action="navigate" data-view="confirm|reschedule|deliver"`
  - `reschedule.js` — list card → `data-action="navigate-with-po"`; back button → `data-action="navigate" data-view="detail"`
  - `deliver.js` — list "Ship more" → `data-action="deliver-open" data-po-id`; form back/header arrow → `data-action="deliver-back"`; stepper +/- → `data-action="deliver-step" data-delta="±1"`; fill-all → `data-action="deliver-fill-all"`; submit → `data-action="deliver-submit"`; list back → `data-action="navigate" data-view="list"`
  - `list.js` — card click drops `window.goToPageWithPO` fallback — uses `data-action="navigate-with-po"` directly. Filter tab listeners remain inline (no global navigation, just renderer-local state).
  - `confirm.js` — already used id-based `addEventListener` via `attachConfirmHandlers()` (no `onclick=` to migrate). Header doc updated.

**Bundle delta**: 54.85 KB → 55.87 KB (+1.02 KB raw / +0.25 KB gzip) — trade-off for declarative dispatch + cleaner architecture. Will shrink in Round 5 once inline JS in Snippet 4 is removed.

**Test count**: 384 → 435 (+51 tests).

- `tests/jest/liff-b2f-maker-bridge.test.js` (NEW) — 48 tests covering: setupEventDelegation contract (null root noop, idempotent cleanup, listener removal), navigate action (click bubbling, missing data-view, unknown action, no-data-action click), navigate-with-po action (po-id forwarding, fallback to navigate), deliver-open (po-id forwarding, missing po-id no-call, async rejection swallowed), deliver-back (async rejection swallowed), deliver-step (data-delta=1/-1/missing/parsing), deliver-fill-all, deliver-submit (rejection swallowed), exception handling (sync throw caught, listener survives crash), and **source-level drift detection** (11 entry.js drift checks + 9 pages/* drift checks — fails CI when V.0.5 cleanup is undone).
- `tests/jest/liff-b2f-maker-pages.test.js` (UPDATED) — 6 assertions migrated from `goToPage('confirm')` literal text scan to `querySelector('button[data-action="navigate"][data-view="confirm"]')` semantic query. 1 assertion `goToPageWithPO('reschedule','99')` → data-attr query. 2 assertions `b2fFillAllRemaining` / `b2fSubmitDeliver` → data-action. 3 NEW assertions for stepper data-delta=±1, deliver-back twice (header + footer), and "no inline onclick survives" sanity check.
...
- Source-level drift detector blocks regressions: any future commit that re-introduces `window.goToPage =` or `onclick="..."` in pages/* fails CI immediately
- DOM contract preserved — same input ids (`#b2f-eta`, `#b2f-note`, `.b2f-dlv-qty`, etc.)
...
#### Phase 1 — Idempotency batch 28 (5 endpoints — 🎯 70% MAJOR MILESTONE + sprint round 50 anniversary)

5 endpoints closed across 2 snippets (cross-snippet 2-file batch):

- `POST /dinoco-slip/v1/replay-slip` (`[Admin System] DINOCO Slip Monitor` V.1.14 → **V.1.15**) — single `{log_id}` only. Admin "Replay" double-click on slow b2b_handle_slip_image cascade (LINE Content download ~1-2s + Slip2Go API ~1-3s) → 2× LINE quota burn + 2× Slip2Go quota burn + possible 2× debt subtract path (d) when LINE Content still fresh; cached 200 replay skips entire cascade.
- `POST /dinoco-slip/v1/review-decision` (Snippet V.1.15) — single `{log_id, decision, delete_image}`. Pool sweep "บันทึก decision" double-click → 2× UPDATE row + 2× audit_log "slip_review_decision" + 2× @unlink image (if not_slip). DB has audit lock review_decision NOT NULL → 409 BUT cached idempotency replay extends across full TTL beyond DB lock so duplicate audit_log row never even attempts INSERT. Different decision mid-retry (is_slip vs not_slip vs manual_process) → 409 catches admin re-evaluation = audit trail integrity defense.
- `POST /dinoco-slip/v1/ai-toggle` (Snippet V.1.15) — boolean+constant-marker `{action:'ai-toggle', explicit:'on'|'off'|'toggle', user_id}`. Admin "🔄 Toggle AI Classifier" double-click within rate window → 2× update_option flicker + 2× audit_log "config_change" + 2× cache invalidation if downstream Slip2Go listener uses option. Different explicit mode mid-retry → 409 catches admin re-evaluation toggle vs explicit ON.
- `POST /dinoco-slip/v1/issue-credit-note` (Snippet V.1.15) — **CRITICAL financial** single `{log_id, amount round(2), notify_dist}` core. reason EXCLUDED (audit annotation only — admin may correct text mid-retry without 409 noise; >=10 chars validation enforces meaningful audit). Admin "💰 ออก Credit Note" double-click on slow b2b_debt_subtract FOR UPDATE + dinoco_audit_log + slip_log UPDATE + LINE push + admin Flex notify chain ~1-3s. DB has GET_LOCK dnc_cn_{log_id} 3s timeout + credit_note_issued_at NOT NULL → 409 inside lock BUT cached replay extends across full TTL beyond GET_LOCK so retry 5s+ later (lock released) skips DB round-trip entirely. Different amount mid-retry → 409 **CRITICAL** prevents wrong debt subtract via cached replay = wrong distributor LINE notify amount + accounting reconciliation drift. Different log_id mid-retry → 409 catches admin clicked wrong overpayment row.
- `POST /dinoco-b2f-audit/v1/purge-stale-prices` (`[Admin System] B2F Migration Audit` V.3.20 → **V.3.21**) — constant-marker `{action:'purge-stale-prices', sentinel, dry_run, user_id}`. **9th constant-marker instance** after R30 stock/initialize + R32 manual-flash-test + R39 daily-summary + R40 dip-stock/start + R43 invoice/init + R47 stock/sync-missing + R49 smoke-test + R49 flag-audit/retention/run — **pattern fully mature across 9 endpoints in 9 rounds**. Admin "🧹 Purge Stale Prices" double-click on slow UPDATE batch (sentinel hits 100+ junction rows + N× do_action 'b2f_junction_updated' cache invalidation ~500ms total) → 2× UPDATE storm + 2× cache flush. Different sentinel mid-retry (admin typed 999 instead of 666) → 409 catches admin re-evaluation. dry_run=1 → live=0 different hashes (preview vs commit semantic distinct). NEW namespace integration — first dinoco-b2f-audit/v1 endpoint wrapped after 19 sibling POSTs; opens cluster for Round 51+.

Backward compat: missing `X-Idempotency-Key` header = byte-identical to V.1.14 / V.3.20 behavior.

**dinoco-slip/v1 namespace coverage 2 → 6/6 = 100% saturation** (second cluster fully saturated after MCP R35). **dinoco-b2f-audit/v1 namespace coverage 0 → 1** (NEW namespace integration — opens cluster for Round 51+; 19 sibling POSTs remain).

Pattern mix: 3× single (replay-slip + review-decision + issue-credit-note) + 1× boolean+constant-marker hybrid (ai-toggle) + 1× constant-marker (purge-stale-prices — 9th instance). Pattern maturity at Round 50 unchanged: **7 patterns** (single / bulk / bulk-of-targets / state-machine / boolean+enum-discriminator / constant-marker / binary-fingerprint).

**Strategic note**: After 🎯 70% MAJOR MILESTONE + sprint round 50 anniversary milestone, **strongly recommend slow-down to 1-2 weeks production canary observation matching Round 42 50% pause pattern**. Sustained 32-round campaign represents significant test infra burden (515 cumulative test cases) + production-traffic validation gap (some integrated endpoints since Round 46-50 have not yet seen a full week of real-traffic retry observation). Round 51 strategic options: (1) **Pause + canary** RECOMMENDED — 1-2 weeks observation; (2) Pivot to OpenAPI bulk doc sweep across 139 wrapped endpoints; (3) Continue toward 80% (157/196 = +18 endpoints) via B2F Migration Audit cluster (sync-missing-intermediates constant-marker / junction-bulk-delete CRITICAL bulk / autosync-blacklist single / junction-update-classification state-machine / junction-confirm-classification idempotent re-confirm).

#### Phase 2 — Tests + drift detector

- `tests/helpers/IdempotencyRound50Test.php` (NEW, 18 tests) — 3-4 cases per endpoint × 5 endpoints + cumulative no-collision (1).
- `docs/audit/IDEMPOTENCY-COVERAGE.md` — coverage 134/196 (68.4%) → **🎯 139/196 (70.9%)**. 5 new rows for entries 136-140. Section heading updated. NEW Round 50 milestone row + Round 51+ candidates rewritten with pause/canary recommendation.
- `tests/jest/php-security.test.js` — RCE_SHELL allowlist line pin updated 999 → **1009** (B2F Migration Audit V.3.21 header insertion +10 LOC for 70% milestone wrap drifted mysqldump shell_exec line).
