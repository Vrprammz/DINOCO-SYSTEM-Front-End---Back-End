...
| REG-071 | G3 BO secondary helper missing — unload Snippet 1 V.34.0+ helper `b2b_flash_create_secondary` then trigger `b2b_bo_items_fulfilled` action | `_flash_bo_pending` meta set with `reason='b2b_flash_create_secondary missing...'`. b2b_log shows `[BO-H5] Flash secondary helper missing`. NO call to `b2b_flash_create_order` (broken fallback removed). Admin sees pending flag for manual Flash creation |
| REG-072 | G3 BO secondary exception path — mock `b2b_flash_create_secondary` to throw → BO fulfill fires → catches Throwable + flags `_flash_bo_pending` with `reason` populated by exception message | Both b2b_log message + meta set. Action listener does not crash subsequent listeners (priority 20 print queue still runs) |
| REG-073 | G4 async snapshot defer — order paid with 50+ items → measure `paid` transition time | Sync listener returns < 100ms (vs ~2-5s pre-fix). `wp_schedule_single_event` queued for `dinoco_v42_snapshot_async`. WP-Cron fires within 30s → snapshot meta populated. b2b_log shows both `scheduled async snapshot` + `snapshotted N entries (async)` |
| REG-074 | G4 async snapshot race — admin clicks Flash Create immediately after `paid` (before async fires) | Dispatcher falls back to `dinoco_resolve_manifest_shipping()` (live resolver) — Flash create succeeds. Async snapshot fires later (idempotency check skips since `_flash_shipping_snapshot` may not exist yet — snapshot still happens). End state: ticket has both PNO + snapshot. `/manual-reprint` works |
| REG-075 | G4 async snapshot dedup — paid fires twice rapidly (e.g. webhook retry) | `wp_next_scheduled` check prevents duplicate event scheduling. Only 1 cron event runs. Idempotency in async handler skips 2nd call if 1st completed. No double snapshot |
| REG-076 | BUG-2 multi-box subParcel JSON — create order with 3+ boxes via V.42 dispatcher → inspect Flash audit `request_summary` | `subParcel` field in audit row stored as JSON string `"[{...},...]"` not PHP array string `"Array"`. Flash response `data.pno` returned + `data.subParcel` array populated (Flash echoes back what it parsed). NO sign error in `b2b_log` |
| REG-077 | BUG-1 insureDeclareValue absence — create non-insured ticket via both V.42 + V.41 paths | `wp_dinoco_flash_audit.request_summary` JSON does NOT contain key `insureDeclareValue` when `insured=0`. Field appears ONLY when `insured=1` with int value (satang). Flash response code=1 (success) — no "invalid format" error |
| REG-078 | ISSUE-5 V.41 returnXXX present — walk-in distributor confirm bill → V.41 fallback path | Flash audit `request_summary` includes 7 `returnXXX` fields populated from `b2b_registered_address` option (registered ลาดพร้าว 15). On simulated return scenario, Flash routes back to registered address, NOT warehouse pickup |
| REG-079 | ISSUE-6 V.41 articleCategory=6 default — admin clears `_flash_article_category` meta + `b2b_warehouse_address.wh_article_category` option → trigger V.41 create | `b2b_flash_get_article_category()` returns 6 (was 99). Audit `request_summary.articleCategory=6`. F2 cron `flash_category_verify_cron` does NOT trigger bump alert (consistent with V.42 default) |
